...
- **신청 후 브랜드 인증(Brand Verification)** — 접수 확인 메일이 오고, 완료하면 심사가 빨라짐. **필수 아님·권장**
  (안 하면 팀이 그냥 ≈5영업일 정상 심사). **에이전트는 GCP에서 대신 진행하지 말고 "방법만 안내"**, 사용자가 원할 때만.
  방법: 개발자토큰↔GCP프로젝트 연결 → OAuth 앱 **외부+프로덕션 게시** → 브랜딩(앱이름·로고·도메인) 입력 →
  **"브랜딩 확인" → "브랜딩 게시"**. **모든 소통은 이메일**(신청서 연락처)로 오니 계속 확인. (프로덕션 게시는 리프레시 토큰 7일 만료도 해소)
